...
def getQuestions(filename, questions, questionLimit):
    ...
    if verbose:
        file = open("categories\\" + filename, "r", encoding="latin_1")
    ...
    for i in range(len(rawQuestions)):
        ...
        question = {"question":     "blank",                                                        ##question dictionary
                    "correct_ans":  "blank",
                    "answer_a":     "blank",
                    "answer_b":     "blank",
                    "answer_c":     "blank",
...
def printQuestions(questions):
    for i in range(len(questions)):
        for k, v in questions[i].items():
            if k == "question": print(str(i+1) + ") ", end="")
            print(v)
...
##writes to SQL file compatible with MySQL
def writeSQL(questions):
    sqlfile = open("qs.sql", "w", encoding="latin_1")
    tablename = "Table"
    sqlfile.write("INSERT INTO " + tablename + " (question, correct_ans, answer_a, answer_b, answer_c, answer_d)\n")
...
##writes to JSON file compatabile with MongoDB
def writeJSON(questions):
    jsonfile = open("qs.json", "w", encoding="latin_1")
    jsonfile.write("db.inventory.insertMany([\n")
    for i in range(len(questions)):
        counter = 1
        line = "    {"
        for k, v in questions[i].items():
...
if __name__ == "__main__":
    ...
    output = True               ##prints the data of the uploaded questions to the console
    numberOfQuestions = 100     ##the number of questions to upload to the database
